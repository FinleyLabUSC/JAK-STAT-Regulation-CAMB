...
for (i in 1:dim(combined_df_sorted)[1]) {
  ...
  # k2 and k11 will be manually labelled
  if (i == 2) {
    combined_df_sorted$row[i] = ""
  }
}
...
b <- b + geom_text(combined_df_sorted, mapping = aes(x=x,y=neg.FB.strength + 0.015 * sign(neg.FB.strength) - 0.0075,label=row),vjust=0,color="black",size=2.5, repel = TRUE)
b <- b +theme(legend.position = "none")
b
ggsave('figures/corr_plot2.pdf',width = 4.5,height = 3)
