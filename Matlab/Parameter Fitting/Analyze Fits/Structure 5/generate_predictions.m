...
pStatA = importdata('cache/pStatA_norm.mat')';
pStatB = importdata('cache/pStatB_norm.mat')';
ncA = importdata('cache/transloc_predA.mat')';
ncB = importdata('cache/transloc_predB.mat')';
Bcl = importdata('cache/Bcl_pred.mat')';
pJAK2 = importdata('cache/pJAK2_norm.mat')';

% Take only first 6 hours from all but BCL
pStatA = pStatA(:,1:361); 
pStatB = pStatB(:,1:361);
ncA = ncA(:,1:361);
ncB = ncB(:,1:361);
...
pJAK2_mean = mean(pJAK2);                     
pJAK2_std = std(pJAK2);
pJAK2_SEM = std(pJAK2)/sqrt(N);              
pJAK2_CI95 = tinv([0.025 0.975], N-1);        
pJAK2_yCI95 = bsxfun(@times, pJAK2_SEM, pJAK2_CI95(:));
...
% Save it
saveas(f,strcat('figures/model_predictions/','pSTATA','.png'))
...
% Save it
saveas(f,strcat('figures/model_predictions/','pSTATB','.png'))
...
% Save it
saveas(f,strcat('figures/model_predictions/','translocation_predictions','.png'))
...
% Save it
saveas(f,strcat('figures/model_predictions/','BCL_fold_change','.png'))



%--- plot pJAK2
f = figure('visible','off');
% [ph,msg]=jbfill(pStat_hours,pJAK2_mean+pJAK2_yCI95(2,:),pJAK2_mean+pJAK2_yCI95(1,:),my_cols(1,:),my_cols(1,:),1,0.2);
[ph,msg]=jbfill(pStat_hours,pJAK2_mean+pJAK2_std,pJAK2_mean-pJAK2_std,my_cols(1,:),my_cols(1,:),1,0.2);
hold on;
plot(pStat_hours,pJAK2_mean,'-','color',my_cols(1,:),'LineWidth',2)
hold on;
% Experimental data
scatter([Data_time_hours(3) Data_time_hours(5) Data_time_hours(6) Data_time_hours(8) Data_time_hours(10) Data_time_hours(12)],Exp_data(34:39),48,'MarkerEdgeColor',my_cols(1,:),'Marker','s','Linewidth',1,'MarkerFaceColor',my_cols(1,:))
...
% Save it
saveas(f,strcat('figures/model_predictions/','pJAK2','.png'))
...
saveas(gcf,strcat('figures/model_predictions/','figS3','.png'))
