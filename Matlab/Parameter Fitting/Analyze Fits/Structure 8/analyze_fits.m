...
num_param_set = 2; % Top x param sets to include (change based on cutoff value from histogram of error values)
...
free_params = zeros(num_iter*1,numParams); % pre-allocate for first one
count_index = 0;
for i = 2
    my_set = my_param_sets(i);
    free_params(count_index+1:num_iter*1,:) = (estimates{my_set}(:,(end-num_iter+1):end))';
