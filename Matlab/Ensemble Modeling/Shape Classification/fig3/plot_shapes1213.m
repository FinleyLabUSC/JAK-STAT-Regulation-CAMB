...
filename = strcat('data/pStat_shape',string(j),'.mat');
...
%% --- plot pStat +/- 95% CI
% figure()
[ph,msg]=jbfill(predTime_hours,pStat_mean+pStat_yCI95(2,:),pStat_mean+pStat_yCI95(1,:),my_cols(j-3,:),my_cols(j-3,:),1,0.2);
hold on;
plot(predTime_hours,pStat_mean,'-','color',my_cols(j-3,:),'LineWidth',2)
hold on;
...
% legend('b','Early','b','Weak FB','b','Late','location','northoutside')
% % Set size of figure
width = 4;
height = 2.5;
set(gcf,'units','inches','position',[1,0.5,width,height])

fig_name = strcat('figures/shape1213','_ci.png');
saveas(gcf,fig_name)
